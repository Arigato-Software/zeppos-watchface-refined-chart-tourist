...
params:
  ...
  zone_func - callback смены зоны давления:
    ...
    4 - повышенное
    5 - высокое
  pressureMode - режим давления:
    0 - абсолютное
    1 - приведенное к уровню моря (по умолчанию)
  data:
...
export class Barometer {
  ...
  constructor(params) {
    ...
    this._pressureMode = params?.pressureMode ?? 1;
  ...
  update() {
    ...
    // Выбор режима: абсолютное или приведенное к уровню моря
    const slp = this._pressureMode === 1 ? this._mmHg2SLP(mmHg, alt) : mmHg;
    ...
    if (this._bar !== bar) {
      ...
      if (this._zone_func) {
        const barZone = this._getBarZone(slp, alt);
  ...
  _getBarZone(slp, alt) {
    ...
    for (; barZone < this._bar_bound.length; barZone++) {
      ...
      let bar_bound = this._bar_bound[barZone];

      // Если режим абсолютного давления (0), пересчитываем границу зоны под текущую высоту
      if (this._pressureMode === 0) {
        bar_bound = this._slp2mmHg(bar_bound, alt);
      }

      if (slp < bar_bound + k) break;
    }
